DROP DATABASE IF EXISTS lesson_4_p2;
CREATE DATABASE lesson_4_p2;
USE lesson_4_p2;
...
union select clients.login from clients;

SELECT users.login from users
union all select clients.login from clients;
...
WHERE EXISTS 
(SELECT * FROM Projects WHERE Employee.Id = Projects.EmployeeId);

SELECT Id FROM Employee
WHERE EXISTS (SELECT EmployeeId FROM Projects WHERE Id = 1004);
...
SELECT DISTINCT MARK,
                (SELECT COUNT(*) FROM AUTO WHERE MARK <> A.MARK) AS COUNT
FROM AUTO A;

### 3. Вывести марку авто - а так же сколько авто такой марки в таблице , вывести записи где количество авто
## такой марки больше 2, записи не должны дублироваться
SELECT A1.MARK, COUNT(A1.MARK) AS COUNT
FROM AUTO A1
WHERE A1.MARK IN (
    SELECT A2.MARK
    FROM AUTO A2
    GROUP BY A2.MARK
    HAVING COUNT(A2.MARK) > 2
)
GROUP BY A1.MARK;
...
SELECT AUTO.MARK, AUTO.COLOR, MAN.FIRSTNAME, MAN.LASTNAME
FROM AUTO 
JOIN MAN  ON AUTO.PHONENUM = MAN.PHONENUM
JOIN CITY ON MAN.CITYCODE = CITY.CITYCODE
WHERE CITY.PEOPLES > 1000000;